...
__global__ void single_iteration_parallel(float *result, float *temp, float *power, int row, int col, float Cap_1, float Rx_1, float Ry_1, float Rz_1, float step) {
...
void compute_tran_temp(float *result, int num_iterations, float *temp, float *power, int row, int col) {
    ...
    size_t matrix_size = row * col * sizeof(float);
    float *r;
    float *t;
    float *p;

    cudaMalloc(&r, matrix_size);
    cudaMalloc(&t, matrix_size);
    cudaMalloc(&p, matrix_size);

    cudaMemcpy(p, power, matrix_size, cudaMemcpyHostToDevice);
    cudaMemcpy(r, result, matrix_size, cudaMemcpyHostToDevice);
    cudaMemcpy(t, temp, matrix_size, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(result, r, matrix_size, cudaMemcpyDeviceToHost);
    cudaMemcpy(temp, t, matrix_size, cudaMemcpyDeviceToHost);    
